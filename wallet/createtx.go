...
import (
	...
	"github.com/palcoin-project/palcwallet/wallet/txsizes"
...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentTotal := palcutil.Amount(0)
	currentInputs := make([]*wire.TxIn, 0, len(eligible))
	currentScripts := make([][]byte, 0, len(eligible))
	currentInputValues := make([]palcutil.Amount, 0, len(eligible))

	return func(target palcutil.Amount) (palcutil.Amount, []*wire.TxIn,
		[]palcutil.Amount, [][]byte, error) {
...
func (s secretSource) GetKey(addr palcutil.Address) (*btcec.PrivateKey, bool, error) {
...
func (s secretSource) GetScript(addr palcutil.Address) ([]byte, error) {
	ma, err := s.Address(s.addrmgrNs, addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, keyScope *waddrmgr.KeyScope,
	account uint32, minconf int32, feeSatPerKb palcutil.Amount,
...
// transaction.
func inputYieldsPositively(credit *wtxmgr.Credit, feeRatePerKb palcutil.Amount) bool {
	inputSize := txsizes.GetMinInputVirtualSize(credit.PkScript)
	inputFee := feeRatePerKb * palcutil.Amount(inputSize) / 1000
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []palcutil.Amount) error {
