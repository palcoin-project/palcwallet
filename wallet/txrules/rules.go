...
const DefaultRelayFeePerKb palcutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb palcutil.Amount) palcutil.Amount {
	...
	relayFee := palcutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount palcutil.Amount, scriptSize int, relayFeePerKb palcutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb palcutil.Amount) bool {
	...
	return IsDustAmount(palcutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb palcutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > palcutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb palcutil.Amount, txSerializeSize int) palcutil.Amount {
	fee := relayFeePerKb * palcutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > palcutil.MaxSatoshi {
		fee = palcutil.MaxSatoshi
	}
