...
func SumOutputValues(outputs []*wire.TxOut) (totalOutput palcutil.Amount) {
	for _, txOut := range outputs {
		totalOutput += palcutil.Amount(txOut.Value)
	}
	return totalOutput
}
...
// InputSourceError.
type InputSource func(target palcutil.Amount) (total palcutil.Amount, inputs []*wire.TxIn,
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []palcutil.Amount
	TotalInput      palcutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, feeRatePerKb palcutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []palcutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := palcutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
