...
func (w *Wallet) MakeMultiSigScript(addrs []palcutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*palcutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*palcutil.AddressScriptHash, error) {
	var p2shAddr *palcutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = palcutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*palcutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
