...
func testImportAccount(t *testing.T, w *Wallet, tc *testCase, watchOnly bool,
	...
	case waddrmgr.NestedWitnessPubKey:
		witnessAddr, err := palcutil.NewAddressWitnessPubKeyHash(
			palcutil.Hash160(acct3ExternalPub.SerializeCompressed()),
		...
		intAddr, err = palcutil.NewAddressScriptHash(
			witnessProg, &chaincfg.TestNet3Params,
		)
		require.NoError(t, err)

	case waddrmgr.WitnessPubKey:
		intAddr, err = palcutil.NewAddressWitnessPubKeyHash(
			palcutil.Hash160(acct3ExternalPub.SerializeCompressed()),
			&chaincfg.TestNet3Params,
		)
		require.NoError(t, err)
