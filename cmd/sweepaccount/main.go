...
var (
	walletDataDirectory = palcutil.AppDataDir("palcwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
func makeInputSource(outputs []btcjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue palcutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		inputValues     = make([]palcutil.Amount, 0, len(outputs))
		sourceErr       error
	)
	for _, output := range outputs {
		output := output

		outputAmount, err := palcutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(palcutil.Amount) (palcutil.Amount, []*wire.TxIn, []palcutil.Amount, [][]byte, error) {
...
func sweep() error {
	...
	var totalSwept palcutil.Amount
	var numErrors int
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := palcutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount palcutil.Amount) bool {
	return amount >= 0 && amount <= palcutil.MaxSatoshi
}
