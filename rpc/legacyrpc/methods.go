...
func unsupported(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &btcjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by palcwallet",
	}
}
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*palcutil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := palcutil.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	secp256k1Addrs := make([]palcutil.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	address, err := palcutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
...
func decodeAddress(s string, params *chaincfg.Params) (palcutil.Address, error) {
	addr, err := palcutil.DecodeAddress(s, params)
	if err != nil {
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	wif, err := palcutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	var (
		debitTotal  palcutil.Amount
		creditTotal palcutil.Amount // Excludes change
		fee         palcutil.Amount
		feeF64      float64
	)
...
func help(icmd interface{}, _ *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	// wallet itself is a websocket client to palcd.  Therefore, create a
...
// and amounts.
func makeOutputs(pairs map[string]palcutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
	outputs := make([]*wire.TxOut, 0, len(pairs))
	for addrStr, amt := range pairs {
		addr, err := palcutil.DecodeAddress(addrStr, chainParams)
		if err != nil {
...
func sendPairs(w *wallet.Wallet, amounts map[string]palcutil.Amount,
	keyScope waddrmgr.KeyScope, account uint32, minconf int32,
	feeSatPerKb palcutil.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	amt, err := palcutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]palcutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pairs := make(map[string]palcutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := palcutil.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := palcutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	var keys map[string]*palcutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*palcutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := palcutil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			addr, err := palcutil.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	case *palcutil.AddressPubKeyHash: // ok
		return bytes.Equal(palcutil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *palcutil.AddressPubKey: // ok
