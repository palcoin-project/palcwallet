...
//
//   https://github.com/palcoin-project/palcwallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://github.com/palcoin-project/palcwallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := palcutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	var totalAmount palcutil.Amount
	for _, output := range unspentOutputs {
		...
		totalAmount += palcutil.Amount(output.Output.Value)

		if req.TargetAmount != 0 && totalAmount > palcutil.Amount(req.TargetAmount) {
			break
		}
	}

	var changeScript []byte
	if req.IncludeChangeScript && totalAmount > palcutil.Amount(req.TargetAmount) {
