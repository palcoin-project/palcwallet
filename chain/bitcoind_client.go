...
//
// NOTE: This is part of the chain.Interface interface.
func (c *BitcoindClient) NotifyReceived(addrs []palcutil.Address) error {
	_ = c.NotifyBlocks()
...
//	[]wire.OutPoint
//	[]*wire.OutPoint
//	map[wire.OutPoint]palcutil.Address
//	[]chainhash.Hash
//	[]*chainhash.Hash
func (c *BitcoindClient) LoadTxFilter(reset bool, filters ...interface{}) error {
	...
	for _, filter := range filters {
		...
		case []palcutil.Address, []wire.OutPoint, []*wire.OutPoint,
			map[wire.OutPoint]palcutil.Address, []chainhash.Hash,
			[]*chainhash.Hash:
...
func (c *BitcoindClient) Rescan(blockHash *chainhash.Hash,
	addresses []palcutil.Address, outPoints map[wire.OutPoint]palcutil.Address) error {
...
func (c *BitcoindClient) rescanHandler() {
	...
	for {
		...
		case update := <-c.rescanUpdate:
			...
			case map[wire.OutPoint]palcutil.Address:
				c.watchMtx.Lock()
				for op := range update {
					c.watchedOutPoints[op] = struct{}{}
				}
