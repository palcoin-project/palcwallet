...
func (s *NeutrinoClient) GetBlock(hash *chainhash.Hash) (*wire.MsgBlock, error) {
	...
	//  Should the block cache be INSIDE neutrino instead of in palcwallet?
...
func (s *NeutrinoClient) Rescan(startHash *chainhash.Hash, addrs []palcutil.Address,
	outPoints map[wire.OutPoint]palcutil.Address) error {
...
func (s *NeutrinoClient) NotifyBlocks() error {
	...
	if !s.scanning {
		s.clientMtx.Unlock()
		return s.NotifyReceived([]palcutil.Address{})
	}
	s.clientMtx.Unlock()
	return nil
}
...
func (s *NeutrinoClient) NotifyReceived(addrs []palcutil.Address) error {
	s.clientMtx.Lock()
...
func (s *NeutrinoClient) onFilteredBlockConnected(height int32,
	header *wire.BlockHeader, relevantTxs []*palcutil.Tx) {
