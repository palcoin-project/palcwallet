...
// Address returns the palcutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() palcutil.Address {
	return a.address
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*palcutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return palcutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	...
	if compressed {
		...
		pubKeyHash = palcutil.Hash160(pubKey.SerializeUncompressed())
	}

	var address palcutil.Address
	var err error
	...
	case NestedWitnessPubKey:
		...
		witAddr, err := palcutil.NewAddressWitnessPubKeyHash(
		...
		address, err = palcutil.NewAddressScriptHash(
	...
	case WitnessPubKey:
		address, err = palcutil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *palcutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() palcutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := palcutil.NewAddressScriptHashFromHash(
